...
if (registerForm) {
  registerForm.onsubmit = async (e) => {
    ...
    const response = await api(form.id, 201, `${window.mailApiHost}/api/v1/auth/register`, {
      method: "POST",
      headers: {
...
if (loginForm) {
  loginForm.onsubmit = async (e) => {
    ...
    const response = await api(form.id, 200, `${window.mailApiHost}/api/v1/auth/authenticate`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
