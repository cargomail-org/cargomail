...
uploadForm.onsubmit = async (e) => {
  ...
  const url = `${window.mailboxApiHost}/api/v1/files/upload`;
...
const filesTable = new DataTable("#filesTable", {
  ...
  ajax: function (data, callback, settings) {
    (async () => {
      const response = await api(uploadForm.id, 200, `${window.mailboxApiHost}/api/v1/files/list`, {
        method: "POST",
        headers: {
  ...
  buttons: [
    // "pageLength",
    {
      ...
      action: function () {
        (async () => {
          const response = await api(uploadForm.id, 200, `${window.mailboxApiHost}/api/v1/files/sync`, {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
            },
...
export const deleteFiles = (e) => {
  ...
  (async () => {
    const response = await api(uploadForm.id, 200, `${window.mailboxApiHost}/api/v1/files/trash`, {
      method: "POST",
      headers: {
