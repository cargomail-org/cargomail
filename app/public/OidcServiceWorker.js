  ...
  return (response) => {
    ...
    return response.json().then((tokens) => {
      ...
      const accessTokenExpiresAt =
        accessTokenPayload && accessTokenPayload.exp ? accessTokenPayload.exp : tokens.issued_at + tokens.expires_in
      ...
      const response = clonedRequest.text().then((actualBody) => {
        if (actualBody.includes(REFRESH_TOKEN) || actualBody.includes(ACCESS_TOKEN)) {
          ...
          if (
            ...
            url.startsWith(currentDatabase.oidcServerConfiguration.revocationEndpoint)
          ) {
